...
// card should be formatted in the FAT32 file system. This firmware
// version is written for the folder-play function of the DFPlayer. This
// means you have to create folders named 01, 02, 03, ... in the SD card
// root and inside the folders you have to name the audio files 001.mp3,
// 002.mp3, ... The maximum number of folders is 99 and the maximum number
// of files per folder is 255. Don't leave gaps in the folders or file
// numbers. After the three-digit number of the MP3 files, the title name
// can be added for better identification, e.g. "003_MySong.mp3". There
// must not be any unnecessary files on the SD card, such as those stored
// there by MacOS (e.g. Spotlight). Better take Linux or Windows to write
...
#define IR_TRK_M      0x06    // IR code for previous track
#define IR_FOL_P      0x04    // IR code for next folder
#define IR_FOL_M      0x07    // IR code for previous folder
...
#define DFP_getFiles()      DFP_query(DFP_QRY_FTRACKS, 0)     // get total files in current folder
...
int main(void) {
  ...
  uint8_t file;                                 // current file number
  uint8_t folder;                               // current folder number
  uint8_t files;                                // total number of files in current folder
  uint8_t folders;                              // total number of folders
  ...
  file   = eeprom_read_byte((const uint8_t*)2); // read last file
  folder = eeprom_read_byte((const uint8_t*)3); // read last folder
  if(volume > 30) volume = 10;                  // set standard value if incorrect reading
  if(!file) file = 1; if(!folder) folder = 1;   // set standard value if incorrect reading

  // Setup IR receiver and DFPlayer
  IR_init();                                    // init IR receiver
  DFP_init();                                   // init DFPlayer
  DFP_setVolume(volume);                        // set volume
  folders = DFP_getFolders();                   // query total number of folders
  if(folder > folders) {                        // if incorrect folder value:
    folder = 1; file = 1; }                     // set file and folder to 1
  DFP_playFolder(folder, 1);                    // just to switch into the folder
  _delay_ms(100);                               // wait for folder access
  files = DFP_getFiles();                       // query total number of files in folder
  if(file > files) file = 1;                    // set to 1 if incorrect file
  DFP_playFolder(folder, file);                 // play track
  ...
  while(1) {
    ...
    if(IR_available()) {
      ...
      eeprom_update_byte((uint8_t*)3, folder);   // update EEPROM (folder)
    }
